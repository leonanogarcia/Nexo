...
new_menu = '''    def _show_mat_col_menu(self, e):
        ...
        items = [s for s in self._mat_header_specs if s[0] not in ('dummy', 'edit', 'delete', 'options', 'barcode')]
        w_menu, h_menu = 180, 40 + len(items) * 32
        
        canvas = tk.Canvas(top, width=w_menu, height=h_menu, bg=bg_color, highlightthickness=0)
