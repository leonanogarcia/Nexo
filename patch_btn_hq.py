...
match = re.search(r'class RoundedActionButton\(tk\.Frame\):.*?def _redraw\(self\):.*?(?=\n\nclass |\Z)', text, re.DOTALL)
if match:
    old = match.group(0)
    ...
    def _make(self,fill, icon=None, start_x=0, cy=0):
        try:
            ...
            if icon:
                ...
                # Draw lines with perfect caps by drawing circles at the ends
                def line(pts):
                ...
                if icon=='plus':
                    ...
                    # Perfect ring using filled circles
                    outer = 7 * scale
                    inner = outer - cw
                    d.ellipse((cx-outer, c_y-outer, cx+outer, c_y+outer), fill=col)
                    d.ellipse((cx-inner, c_y-inner, cx+inner, c_y+inner), fill=fill) # Cut out center
                    # Hands
    ...
    print("Patched RoundedActionButton")
else:
    print("Regex failed to find RoundedActionButton")
