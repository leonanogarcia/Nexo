...
# We need to insert the 'inactive' tag in rec_tree.insert
text = re.sub(
    r"iid=self\.rec_tree\.insert\('','end',text='☐',values=\(r\['code'\],name_disp,fmt_num\(r\['yield_qty'\]\),r\['yield_unit'\] or '-',fmt\(cost\),'✏️','🗑️'\)\)",
    r"iid=self.rec_tree.insert('','end',text='☐',values=(r['code'],name_disp,fmt_num(r['yield_qty']),r['yield_unit'] or '-',fmt(cost),'✏️','🗑️'), tags=() if is_active else ('inactive',))",
    text
)
...
# We need to insert the 'inactive' tag in prod_tree.insert
text = re.sub(
    r"iid=self\.prod_tree\.insert\('','end',text='☐',values=\(r\['code'\],name_disp,fmt_num\(r\['weight'\]\),fmt\(cost\),fmt\(r\['price'\]\),mg,'✏️','🗑️'\)\)",
    r"iid=self.prod_tree.insert('','end',text='☐',values=(r['code'],name_disp,fmt_num(r['weight']),fmt(cost),fmt(r['price']),mg,'✏️','🗑️'), tags=() if is_active else ('inactive',))",
    text
)

with open('main.py', 'wb') as f:
    f.write(text.encode('utf-8'))
print("Patched recipes and products tables to include filters!")
